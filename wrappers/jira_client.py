...
class JiraClient(object):
    ...
    def init_jira_session(self):
        ...
        session.auth = HTTPBasicAuth(self.jira_account_email, self.jira_api_token)
        ...
        if resp.status_code == 200:
            ...
            raise Exception(
                ...
                "maybe you need to answer a security question: %s" % self.jira_url
            )
        else:
            raise Exception(
                "Something went wrong," " Jira gave %s status code." % resp.status_code
            )
    ...
    def repair_estimate(self, issue):
        ...
        try:
            ...
            payload = {
                "fields": {
                    "timetracking": {
                        ...
                        "remainingEstimate": self.convert_seconds_to_jira_time(
            ...
            url = urljoin(self.jira_url, f"/rest/api/latest/issue/{issue}")
            self.jira_session.put(url, json=payload)
        except KeyError:
            raise Exception(
                f"repair_estimate: impossible to edit remaining estimate for {issue}. Check permissions and jira workflow. Maybe the `originalEstimate` field is not editable."
            )
